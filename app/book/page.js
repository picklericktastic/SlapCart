...
import { ChevronRight, ChevronLeft, Calendar, MapPin, User, CreditCard, CheckCircle, Clock, AlertCircle, Loader2 } from "lucide-react";
...
export default function BookPage() {
  ...
  // Initialize the Square card form when user reaches Step 4
  useEffect(() => {
    if (step !== 4 || cardAttached.current) return;

    const initSquare = async () => {
      if (!window.Square) {
        setTimeout(initSquare, 400);
        return;
      }
      try {
            ...
            {STEPS.map((s, i) => (
              <div key={s.id} className="flex items-center flex-1">
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className={`text-xs font-medium hidden sm:block ${step === s.id ? "text-brand-blue" : step > s.id ? "text-green-600" : "text-gray-400"}`}>
                    {s.label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <div className="flex items-start gap-2 mb-4 bg-amber-50 border border-amber-200 rounded-xl p-3">
                  ...
                  <p className="text-amber-700 text-xs">
                    Drop-off only available <strong>8+ hours before kickoff</strong> to avoid stadium traffic.
                  </p>
                </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <ZoneMap
              selectedZone={booking.zone}
              onZoneSelect={(z) => { update("zone", z); update("spot", null); }}
              selectedSpot={booking.spot}
              onSpotSelect={(s) => update("spot", s)}
            />
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            {booking.isGuest === null && (
              <div className="grid sm:grid-cols-2 gap-4">
                <button onClick={() => update("isGuest", false)}
                  className="p-5 rounded-2xl border-2 border-gray-200 bg-white hover:border-brand-blue text-left transition-all hover:shadow-md">
                ...
                <button onClick={() => update("isGuest", true)}
                  className="p-5 rounded-2xl border-2 border-gray-200 bg-white hover:border-brand-gold text-left transition-all hover:shadow-md">
            ...
            {booking.isGuest !== null && (
              <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
                <button onClick={() => update("isGuest", null)} className="text-sm text-brand-blue hover:underline flex items-center gap-1">
                  <ChevronLeft className="w-4 h-4" /> Change
                </button>
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">First Name *</label>
                    <input type="text" value={booking.firstName} onChange={(e) => update("firstName", e.target.value)}
                      className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-brand-blue focus:outline-none" placeholder="Mike" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Last Name *</label>
                    <input type="text" value={booking.lastName} onChange={(e) => update("lastName", e.target.value)}
                      className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-brand-blue focus:outline-none" placeholder="Walker" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Email Address *</label>
                  <input type="email" value={booking.email} onChange={(e) => update("email", e.target.value)}
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-brand-blue focus:outline-none" placeholder="mike@example.com" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Phone Number *</label>
                  <input type="tel" value={booking.phone} onChange={(e) => update("phone", e.target.value)}
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-brand-blue focus:outline-none" placeholder="(225) 555-0100" />
                </div>
                {!booking.isGuest && (
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Create Password</label>
                    <input type="password" value={booking.password} onChange={(e) => update("password", e.target.value)}
                      className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-brand-blue focus:outline-none" placeholder="Min. 8 characters" />
                  </div>
                )}
              </div>
            )}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              <div className="bg-brand-blue px-5 py-3">
                <p className="text-white font-bold">Booking Summary</p>
              </div>
              <div className="p-5 space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500 flex items-center gap-1.5"><Calendar className="w-4 h-4" /> Game</span>
                  <span className="font-semibold text-gray-800">LSU vs. {selectedGame?.opponent}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 flex items-center gap-1.5"><Clock className="w-4 h-4" /> Drop-Off</span>
                  <span className="font-semibold text-gray-800">{booking.dropoffTime}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 flex items-center gap-1.5"><MapPin className="w-4 h-4" /> Location</span>
                  <span className="font-semibold text-gray-800">{booking.spot}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 flex items-center gap-1.5"><User className="w-4 h-4" /> Name</span>
                ...
                <div className="border-t border-gray-100 pt-3 flex justify-between text-base">
                  ...
                  <span className="font-black text-brand-blue text-lg">$1,000.00</span>
                </div>
              </div>
            </div>

            <div className="bg-white rounded-2xl border border-gray-200 p-5">
              <p className="font-bold text-brand-blue mb-1 flex items-center gap-2">
                <CreditCard className="w-5 h-5 text-brand-gold" /> Secure Payment
              </p>
              ...
              <button
                ...
                disabled={!cardInstance || isProcessing}
                className={`w-full flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-bold text-base transition-all ${
                  cardInstance && !isProcessing
                    ? "bg-brand-blue text-white hover:bg-brand-blue-light shadow-md hover:shadow-lg hover:-translate-y-0.5"
              ...
              >
                {isProcessing ? (
                  ...
                  <><CreditCard className="w-5 h-5" /> Pay $1,000.00 Securely</>
                )}
              </button>
        ...
        <div className="flex justify-between mt-8 pt-6 border-t border-gray-100">
          {step > 1 ? (
            <button onClick={() => setStep(step - 1)} className="flex items-center gap-2 text-gray-500 hover:text-brand-blue font-semibold transition-colors px-4 py-2">
              <ChevronLeft className="w-5 h-5" /> Back
            </button>
          ) : (
            <Link href="/" className="flex items-center gap-2 text-gray-500 hover:text-brand-blue font-semibold transition-colors px-4 py-2">
